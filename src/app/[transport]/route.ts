...
import { MetaAdsClient } from "@/lib/meta";
  ...
  if (!creds) {
    return new Response(
      JSON.stringify({
        error: "not_connected",
        error_description:
          "No Meta Ads credentials found. Please reconnect via the OAuth flow.",
      }),
  ...
  if (creds.tokenExpiresAt < Date.now()) {
    return new Response(
      JSON.stringify({
        error: "token_expired",
        error_description:
          "Meta access token has expired. Please reconnect via the OAuth flow.",
      }),
      { status: 403, headers: { "Content-Type": "application/json" } }
    );
  }

  const client = new MetaAdsClient(userId);
